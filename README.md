...
You'll need to create your own Google Cloud OAuth client (Google requires this
for desktop apps; takes about ten minutes once).
...
5. Replace `internal/auth/credentials.json` in this repo with the downloaded
   file (overwrite the placeholder).
6. Build and run:
   ...
   The first run opens your browser for consent. After approval the refresh
   token is saved at `~/Library/Application Support/gcal/token.json` (mode
   0600). Subsequent runs skip straight to the TUI.
